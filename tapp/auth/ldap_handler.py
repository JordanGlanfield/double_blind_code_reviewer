...
class LdapConnectionHandler:
    ...
    def ldap_login(self, username, password, query_attrs):
        ...
        attributes = parse_ldap_attributes(self.raw_attributes(username, query_attrs, connection))
        connection.unbind_s()
        return attributes

    def raw_attributes(self, username, attributes, connection):
        ldap_filter = USERNAME_FILTER_TEMPLATE % username
        raw_res = connection.search(self.base_dn, ldap.SCOPE_SUBTREE, ldap_filter, attributes)
...
def ldap_attributes_to_dictionary(attr_values):
    items = (re.findall(KEY_VAL_ATT_REGEX, item.decode('utf-8').replace(',', ' ')) for item in attr_values)
    d = defaultdict(set)
